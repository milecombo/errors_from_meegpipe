...
% meegpipe_master_12may15/+meegpipe/+node/+filter/@filter
import goo.globals;
import misc.eta;
...
%%%%%%%%%%%%%%
    ...
    try 
        scrapOnset= data(first-delay+1:first,:); % prev chunk of delay len
    catch
        %if segItr==1 %|| len_previous < len_delay
        scrapOnset= fliplr(data(first:first+delay-1,:)); % fliplr onset
    end
    %%%%%%%% m1k
    ...
    else % NORMAL CHANNELS  %%%%%%%%%%%%%%  M1k
        if do_reporting(obj)
            pcs = copy(data); 
        else
           pcs = data; %pcs = copy(data);
        end
    end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  M1k
...
%%%%%%%%%%%%%%
pcs = filtfilt(filtObj, pcs, scrapOnset); % it's an alias that redirect to meegpipe's filter and filtfilt
%%%%%%%%%%%%%%    
    ...
    if retRes,
        ...
        data = assign_values(data, pcs, verbose);
    end
    if verbose,
        fprintf(['[done] in ' num2str(toc) ' sec \n\n']);
    end
    %%%%%%%%%%%%%%%%%%%%%%%
